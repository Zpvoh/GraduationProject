...
public class RelevanceRecommendationStrategy implements RecommendationStrategy {
    private double theta = 1;
    private double alpha = 0.5;

    public RelevanceRecommendationStrategy() {
    }

    public RelevanceRecommendationStrategy(double theta, double alpha) {
        this.theta = theta;
        this.alpha = alpha;
    }
    ...
    public ImportanceSortedList useStrategy(PrecursorGraph precursorGraph, EvaluationList evaluationList) {
        ...
        for (VertexWithValue v : N_n_sorted) {
            ...
            double sum_P = 0;
            double N_total_node = 0;
            double N_total_successor_avg = 0;

            List<Integer> scoreListNode = evaluationList.getScoreList().getScoreTotal().get(precursorGraph.getIndexById(v.getVertex().getLongId()));
            for (Integer n : scoreListNode) {
                N_total_node += n;
            }
            ...
            for (VertexWithValue successor : successors) {
                ...
                List<Integer> scoreListSuccessor = evaluationList.getScoreList().getScoreTotal().get(precursorGraph.getIndexById(successor.getVertex().getLongId()));
                for (Integer n : scoreListSuccessor) {
                    N_total_successor_avg += n;
                }
            }

            N_total_successor_avg = successors.size() == 0 ? 0 : N_total_successor_avg / successors.size();
            ...
            double importance = importance_n * (N_total_node + alpha * N_total_successor_avg * delta_importance_avg);
